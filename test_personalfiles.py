...
def guess_flight_angle(data):
    ...
    along_swath_angle = np.pi - np.arctan2((c - c1) - c2_row, d2 - d2_row)
...
def orthorectify():
    ...
    src_file_source = root+'18.22.32_rot180/'  # aerial image
    # ext_dir = root + "single_entries210339pitch6.7/"
    ext_dir = root + "Qinertia/single_entries182232/"
    ...
    int_param_file = root + "int_camera2.yaml"  # interior parameters
    ...
    # create Ortho object and orthorectify
        ...
        os.makedirs(root+f"results_182232/", exist_ok=True)
        ortho.process(root+f"results_182232/ortho_{ii}.tif", resolution=estimate_res, interp=oty.enums.Interp.nearest)
...
def combine_orthosv2(num = "20", stripwidth = 24):
    """combines the orthorectified images through the the centerlines of the overlapping pixels"""
    # root_dir = "C:/Users/thijs/OneDrive/Documents/PostDoc_UPC/Data_local/Mountain_Fire_processing/day_09_04_2022/"
    # root = root_dir+'Metashape/Workswell/Metashape_data_references/Qinertia/' #Qinertia/ Interpolated/
    root_dir = "C:/Users/thijs/OneDrive/Documents/PostDoc_UPC/Data_local/Test_Fire_processing/"
    root = root_dir #+ "Flir_wide/"
    orthos_uint8 = [ii for ii in os.listdir(root+f"results_182232/") if ii.endswith('.tif') and "trimmed" not in ii]
    # orthos = [WarpedVRT(ortho, dtype="float32") for ortho in orthos_uint8]
    orthos_trimmed = []
    for ortho_name in orthos_uint8:
        ortho = r_open(root+f"results_182232/{ortho_name}", "r")
        meta = ortho.meta
        ...
        with rio.open(root+f"results_182232/trimmed_{ortho.name.split("/")[-1]}", 'w', **meta) as dst:
            dst.write(data, indexes=1)
        ortho = rio.open(root+f"results_182232/trimmed_{ortho.name.split("/")[-1]}", "r")
    ...
    # num = "21"
    with rio.open(root+f"ortho_combined_mean_strip{num}.tif", 'w', **output_meta) as dst:
        dst.write(mean)
    with rio.open(root+f"ortho_combined_sum_strip{num}.tif", 'w', **output_meta) as dst:
        dst.write(sum)
    with rio.open(root+f"ortho_combined_counts_strip{num}.tif", 'w', **output_meta) as dst:
        dst.write(counts)
    with rio.open(root+f"ortho_combined_max_strip{num}.tif", 'w', **output_meta) as dst:
        dst.write(max)
    return 0
...
#     print(640*514)
    ...
    num = "182232_v2"
    # combine_orthos()
    combine_orthosv2(num=num, stripwidth=30)
    return 0
